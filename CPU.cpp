...
void CPU::LddAHlpos(uint8_t opcode) {
	_reg->Write8(_reg, A, _mmu->ReadMemory8(_mmu, _reg->Read16(_reg, HL)));
	_reg->Write16(_reg, HL, _reg->Read16(_reg, HL) + 1);
}

//Put A in the address HL, then Increment HL. [8 cycles]
void CPU::LdiHlA(uint8_t opcode) {
...
void CPU::Cb(uint8_t opcode) {
	...
	uint8_t cb_opcode = _mmu->ReadMemory8(_mmu, _reg->pc_first); 
	switch (cb_opcode) {
